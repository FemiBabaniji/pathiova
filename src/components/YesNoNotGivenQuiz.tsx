...
interface YesNoNotGivenQuizProps {
  questionData: {
    passage: string
    questions: Question[]
  }
...
export default function YesNoNotGivenQuiz({
  questionData,
  currentQuestionIndex,
  onAnswerSubmit,
  onNavigate,
  userAnswer
}: YesNoNotGivenQuizProps) {
      ...
      <CardContent className="p-6 bg-transparent">
        <div className="flex flex-col lg:flex-row lg:justify-between">
          ...
          <div className="w-full lg:w-[40%] space-y-6">
            <div className="p-4">
            <p className="text-lg font-semibold -mb-5">Is the statement Yes, No, or Not Given?</p>
            </div>
            <RadioGroup value={selectedAnswer || ""} onValueChange={handleAnswerChange} className="space-y-3">
              {options.map((option) => (
                ...
                >
                  ...
                  <Label
                    htmlFor={option}
                    className="pl-8 cursor-pointer flex-grow text-base font-medium"
                  >
                    {option}
                  </Label>
                </div>
